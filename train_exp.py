...
from WGANLoss import WGANLoss, calculate_gradient_penalty
...
# model parameters
opt.num_workers = 1 # recommend small num_worker
...
# num_images = 800000 # use 80000 images for training one iteration
num_images = 50000 # number of images in one training iteration
...
dynamic_batch_size = [16, 16, 16, 16, 16, 10, 6, 3, 2] # predefined batch size to avoid excceeding gpu memory
    ...
    while image_step < num_images:
        ...
        for j, batch in enumerate(bar):
            ...
            if stablize:
                ...
                alpha = float(min(image_step, num_images) / num_images)
            ...
            # loss_drift = torch.mean(out_fake ** 2) + torch.mean(out_true ** 2) ** epsilon_drift
